(function($) {
    ...
    quotes[0] = "No Bad Debts";
    quotes[1] = "Reduce Email by 20%";
    quotes[2] = "25% Increase in New Logos";
    quotes[3] = "Reduce Dependency on Co-loaders by 20%";
    quotes[4] = "Increase Existing Business by 15%";
    quotes[5] = "Protect the Network";
    quotes[6] = "Less than 3% A/R over 60 days";
    quotes[7] = "100% Quality Billing";
    quotes[8] = "No Bad Debts";
    quotes[9] = "Reduce Email by 20%";

    function getQuote() {
        $(".obtuse").html("<p>" + quotes[getRandomInt(0, (quotes.length - 1))] + "</p>");
    }
        ...
        return this.each(function() {
            ...
            function handleOut(target, pageX, pageY) {
                ...
                $(target).animate({
                    'left': -leftAt_x,
                    'top': -leftAt_y,
                    'rotateZ': deg2rad(-leftAt_r),
                    'scale': (1 - (leftAt_y / 500))
                }, 100);
                $(target).animate({
                    'left': leftAt_x * .9,
                    'top': leftAt_y * .9,
                    'rotateZ': deg2rad(leftAt_r * .9),
                    'scale': (1 + (leftAt_y / 500 * .9))
                }, 100);
                $(target).animate({
                    'left': -leftAt_x * .75,
                    'top': -leftAt_y * .66,
                    'rotateZ': deg2rad(-leftAt_r * .66),
                    'scale': (1 - (leftAt_y / 500 * .66))
                }, 100);
                $(target).animate({
                    'left': leftAt_x * .5,
                    'top': leftAt_y * .5,
                    'rotateZ': deg2rad(leftAt_r * .5),
                    'scale': (1 + (leftAt_y / 500 * .5))
                }, 100);
                $(target).animate({
                    'left': -leftAt_x * .25,
                    'top': -leftAt_y * .25,
                    'rotateZ': deg2rad(-leftAt_r * .25),
                    'scale': (1 - (leftAt_y / 500 * .25))
                }, 100);
                $(target).animate({
                    'left': 0,
                    'top': 0,
